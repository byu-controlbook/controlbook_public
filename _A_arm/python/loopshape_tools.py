...
def print_margins(sys, label, dB_flag=False):
    """
    Print gain and phase margins for a given system "sys" with a label
    "label". If dB_flag is true, gain margin is printed in dB.
    """
...
def add_spec_noise(gamma_n, omega_n, dB_flag=False):
    ...
    if dB_flag == False:
        fig.axes[0].scatter(
            omega_n, gamma_n, facecolors="none", edgecolors="red", label="noise spec"
        )
    else:
        fig.axes[0].scatter(
            omega_n,
            20.0 * np.log10(gamma_n),
            facecolors="none",
            edgecolors="red",
            label="noise spec",
        )
...
def add_spec_input_disturbance(gamma_d, omega_d, system, dB_flag=False):
    ...
    """
    fig = plt.gcf()
    mag, phase, omega = cnt.frequency_response(system, omega=omega_d)
    if dB_flag == False:
        fig.axes[0].scatter(
            omega_d,
            1.0 / gamma_d * mag,
...
def add_spec_ref_tracking(gamma_r, omega_r, dB_flag=False):
    ...
    if dB_flag == False:
        fig.axes[0].scatter(
            ...
            facecolors="none",
            edgecolors=[1, 0, 1],
            label="ref tracking spec",
        )
    else:
        fig.axes[0].scatter(
            omega_r,
            20.0 * np.log10(1.0 / gamma_r),
            facecolors="none",
            edgecolors=[1, 0, 1],
            label="ref tracking spec",
        )
