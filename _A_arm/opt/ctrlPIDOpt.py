...
class ctrlPID:
    ...
    def reset(self, kp=np.nan, ki=np.nan, kd=np.nan):
        # reset the control and change gains.  To be used in the optimization routine
        # reset all stored quantities
        self.theta_dot = 0.0
        self.theta_d1 = 0.0
        self.error_dot = 0.0
        self.error_d1 = 0.0
        self.integrator = 0.0
        # set gains if passed into function
        if np.isnan(kp)==False:
            self.kp = kp
        if np.isnan(ki)==False:
            self.ki = ki
        if np.isnan(kd)==False:
            self.kd = kd
