...
class ctrlPID:
    def __init__(self):
        ...
        M = 15.0  # Time scale separation between inner and outer loop
    ...
    def update(self, phi_r, y):
        ...
        theta_r_unsat = self.kp_phi * error_phi + self.ki_phi * self.integrator_phi - self.kd_phi * self.phi_dot
        # saturate theta_r
        theta_r = saturate(theta_r_unsat, self.theta_max)
        # integrator anti - windup
        if self.ki_phi != 0.0:
            self.integrator_phi = self.integrator_phi + P.Ts / self.ki_phi * (theta_r - theta_r_unsat)
        #---------------------------------------------------
