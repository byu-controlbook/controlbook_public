...
tr_th = 1.0
wn_th = 0.5 * np.pi / (tr_th * np.sqrt(1 - zeta_th**2)) # this is because zeta is not 0.707

a0_th = 0.0
a1_th = 0.0
b0_th = 1/(P.Js + P.Jp)
kp_th = (wn_th**2 -a0_th) / b0_th 
kd_th = (2*zeta_th * wn_th - a1_th) / b0_th

# DC gain for inner loop, limit of P_in*C_in/(1+P_in*C_in) as s->0:
k_DC_th = kp_th*b0_th/(kp_th*b0_th)
...
wn_phi = 0.5 * np.pi / (tr_phi * np.sqrt(1 - zeta_phi**2)) # this is because zeta is not 0.707


# this method of solution (as shown in book) is necessary because kd_phi and kp_phi show
# up in multiple terms (i.e. they are not isolated like our other examples). 
AA = np.array([
